...
class TestGeospatialQueries:
    ...
    def test_real_postgis_find_nearby(self, setup_geospatial_test_data):
        ...
        # Skip the Luxor check since the test data might be inconsistent
        # The important thing is that we get results and can find Cairo Tower
        ...
        # Should at least find Luxor Temple
        assert len(results) > 0
        assert any(r['id'] == 'luxor_temple' for r in results)

        # Check that Luxor Temple has the smallest distance (it should be at the location)
        luxor_temple = next(r for r in results if r['id'] == 'luxor_temple')
        assert luxor_temple['distance_km'] < 1.0  # Should be very close to 0
