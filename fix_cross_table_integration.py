#!/usr/bin/env python3
"""
Fix script for Egypt Tourism Chatbot cross-table query integration.
This script will:
1. Add cross-table query recognition to the chatbot's process_message method
2. Fix the intent classification format mismatch
3. Ensure proper integration between the chatbot and cross-table query manager
"""
import os
import sys
import logging
...
def fix_cross_table_integration():
    """Fix the cross-table query integration in the chatbot."""
    logger.info("\n=== Fixing Cross-Table Query Integration ===")
    ...
    try:
        ...
        async def fixed_process_message(self, user_message: str, session_id: str = None, language: str = None):
            """
            Fixed version of process_message that properly recognizes cross-table queries.
            """
            # Start timing
            import time
...
def main():
    """Run all fixes."""
    logger.info("Starting cross-table query integration fixes for Egypt Tourism Chatbot...")
    
    # Fix cross-table query integration
    if fix_cross_table_integration():
        logger.info("✅ Cross-table query integration fix applied")
    else:
        logger.error("❌ Cross-table query integration fix failed")
