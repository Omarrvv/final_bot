-- Migration: 021_standardize_users_id.sql
-- Purpose: Standardize ID for users table and update foreign key references
...
-- 4. Check for tables referencing users.id
DO $$
DECLARE
    ref_table text;
    ref_column text;
BEGIN
    ...
    LOOP
        ...
        IF EXISTS (
            ...
            EXECUTE format('
                UPDATE %I
                SET %I_integer = (SELECT new_id FROM id_mapping_users WHERE old_id = %I::text)
                WHERE %I IS NOT NULL;
...
BEGIN
    ...
    LOOP
        ...
        IF EXISTS (
            ...
            EXECUTE format('
                ALTER TABLE %I
                ADD CONSTRAINT %I FOREIGN KEY (%I) REFERENCES users(id) ON UPDATE CASCADE ON DELETE SET NULL;
