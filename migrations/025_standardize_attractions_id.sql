-- Migration: 025_standardize_attractions_id.sql
-- Purpose: Standardize ID for attractions table and update foreign key references
...
-- 4. Check for tables referencing attractions.id
DO $$
DECLARE
    ref_table text;
    ref_column text;
BEGIN
    FOR ref_table, ref_column IN
        ...
        WHERE tc.constraint_type = 'FOREIGN KEY'
          AND ccu.table_name = 'attractions'
    ...
    LOOP
        ...
        IF EXISTS (
            ...
            EXECUTE format('
                UPDATE %I
                SET %I_integer = (SELECT new_id FROM id_mapping_attractions WHERE old_id = %I::text)
                WHERE %I IS NOT NULL;
...
ALTER TABLE attractions DROP CONSTRAINT IF EXISTS attractions_pkey;
...
BEGIN
    ...
    LOOP
        ...
        IF EXISTS (
            ...
            EXECUTE format('
                ALTER TABLE %I
                ADD CONSTRAINT %I FOREIGN KEY (%I) REFERENCES attractions(id) ON UPDATE CASCADE ON DELETE SET NULL;
