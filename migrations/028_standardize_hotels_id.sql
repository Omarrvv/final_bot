-- Migration: 028_standardize_hotels_id.sql
-- Purpose: Standardize ID for hotels table and update foreign key references

-- Begin transaction
BEGIN;

-- 1. Create a mapping table
CREATE TABLE id_mapping_hotels (
    old_id text PRIMARY KEY,
    new_id serial NOT NULL
);

-- 2. Populate mapping table
INSERT INTO id_mapping_hotels (old_id)
SELECT id FROM hotels ORDER BY id;

-- 3. Add new integer ID column
ALTER TABLE hotels ADD COLUMN integer_id serial;

-- 4. Check for tables referencing hotels.id
DO $$
DECLARE
    ref_table text;
    ref_column text;
BEGIN
    ...
    LOOP
        ...
        IF EXISTS (
            ...
            EXECUTE format('
                UPDATE %I
                SET %I_integer = (SELECT new_id FROM id_mapping_hotels WHERE old_id = %I::text)
                WHERE %I IS NOT NULL;
...
ALTER TABLE hotels DROP CONSTRAINT IF EXISTS hotels_pkey;

-- 6. Drop old ID column
ALTER TABLE hotels DROP COLUMN id;

-- 7. Rename new ID column
ALTER TABLE hotels RENAME COLUMN integer_id TO id;
...
BEGIN
    ...
    LOOP
        ...
        IF EXISTS (
            ...
            EXECUTE format('
                ALTER TABLE %I
                ADD CONSTRAINT %I FOREIGN KEY (%I) REFERENCES hotels(id) ON UPDATE CASCADE ON DELETE SET NULL;
