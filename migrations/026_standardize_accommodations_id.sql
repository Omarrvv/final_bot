-- Migration: 026_standardize_accommodations_id.sql
-- Purpose: Standardize ID for accommodations table and update foreign key references
...
-- 4. Check for tables referencing accommodations.id
DO $$
DECLARE
    ref_table text;
    ref_column text;
BEGIN
    FOR ref_table, ref_column IN
        ...
        WHERE tc.constraint_type = 'FOREIGN KEY'
          AND ccu.table_name = 'accommodations'
    ...
    LOOP
        ...
        IF EXISTS (
            ...
            EXECUTE format('
                UPDATE %I
                SET %I_integer = (SELECT new_id FROM id_mapping_accommodations WHERE old_id = %I::text)
                WHERE %I IS NOT NULL;
...
ALTER TABLE accommodations DROP CONSTRAINT IF EXISTS accommodations_pkey;
...
BEGIN
    ...
    LOOP
        ...
        IF EXISTS (
            ...
            EXECUTE format('
                ALTER TABLE %I
                ADD CONSTRAINT %I FOREIGN KEY (%I) REFERENCES accommodations(id) ON UPDATE CASCADE ON DELETE SET NULL;
