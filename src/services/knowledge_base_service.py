...
# REMOVED: from src.repositories.repository_factory import RepositoryFactory  # Archived - using unified service provider
from src.core.container import container
# PHASE 0 FIX: Enhanced service registry archived - using unified service provider
...
class KnowledgeBaseService:
    ...
    @property
    def db_service(self):
        """Get the database service for compatibility with integration tests."""
    ...
    def _initialize_new_architecture(self) -> None:
        ...
        try:
            ...
            # REMOVED: self._repository_factory = RepositoryFactory(self._db_core)  # Archived - using unified service provider
            self._repository_factory = None  # Placeholder - repositories accessed via container
            
            # PHASE 0 FIX: Enhanced service registry archived - using unified service provider  
            # self._service_registry = EnhancedServiceRegistry(self.db_manager)
            self._service_registry = None  # Placeholder - services accessed via container
    ...
    def search_restaurants(self, query: Dict = None, limit: int = 10, language: str = "en") -> List[Dict]:
        ...
        try:
            if self._repository_factory:
                ...
                # Fallback to legacy database manager
                result = self.db_manager.search_restaurants(query, {}, limit, 0, language)
            ...
            # Final fallback to db_manager
            if hasattr(self.db_manager, 'search_restaurants'):
                return self.db_manager.search_restaurants(query, {}, limit, 0, language)
            return []
    ...
    def search_hotels(self, query: Dict = None, limit: int = 10, language: str = "en") -> List[Dict]:
        ...
        try:
            if self._repository_factory:
                ...
                # Fallback to legacy database manager
                result = self.db_manager.search_hotels(query, {}, limit, 0, language)
            ...
            # Final fallback to db_manager
            if hasattr(self.db_manager, 'search_hotels'):
                return self.db_manager.search_hotels(query, {}, limit, 0, language)
            return []
