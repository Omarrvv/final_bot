...
class DatabaseManagerService:
    ...
    def __init__(self, database_uri: str = None, vector_dimension: int = 768):
        ...
        # REMOVED: self._repository_factory = RepositoryFactory(db_core)  # Using unified service provider
    ...
    def search_accommodations(self, query: Optional[Dict[str, Any]] = None, filters: Optional[Dict[str, Any]] = None,
        ...
        if query:
            if isinstance(query, dict):
                ...
                search_filters["text"] = str(query)
        # 🔧 CRITICAL FIX: Search "hotels" table instead of empty "accommodations" table
        return self.generic_search("hotels", search_filters, limit, offset, ["name", "description"], language)
