...
class PostgresqlDatabaseManager:
    ...
    def search_attractions(
        ...
        if "name" in filters:
            query += " AND (name_en ILIKE %s OR name_ar ILIKE %s)"
    ...
    def update_attraction_embedding(
        self, attraction_id: str, embedding: List[float]
    ) -> bool:
        """
    ...
    def update_city_embedding(
        self, city_id: str, embedding: List[float]
    ) -> bool:
        """
        ...
        try:
            ...
            if text_query:
                if table == "attractions":
                    query_parts.append("AND (name_en ILIKE %s OR name_ar ILIKE %s OR description_en ILIKE %s OR description_ar ILIKE %s)")
                    ...
                    query_parts.append("AND (name_en ILIKE %s OR name_ar ILIKE %s OR description_en ILIKE %s OR description_ar ILIKE %s OR cuisine ILIKE %s)")
                    text_param = f"%{text_query}%"
